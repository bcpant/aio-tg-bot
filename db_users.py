...
class Db:
    ...
    def add_true_livesansw(self, user_id):
        self.cursor.execute("SELECT `livesModeRight` FROM `users` WHERE `usertgid` = ?", (user_id,))
        result = self.cursor.fetchone()
        if result:
            oldValue = result[0]
            newValue = oldValue + 1
    ...
    def add_wrong_livesansw(self, user_id):
        self.cursor.execute("SELECT `livesModeWrong` FROM `users` WHERE `usertgid` = ?", (user_id,))
        result = self.cursor.fetchone()
        if result:
            oldValue = result[0]
            newValue = oldValue + 1
    ...
    def get_true_livesansw(self, user_id):
        self.cursor.execute("SELECT `livesModeRight` FROM `users` WHERE `usertgid` = ?", (user_id,))
        result = self.cursor.fetchone()
        if result:
            return result[0]
    ...
    def get_wrong_livesansw(self, user_id):
        self.cursor.execute("SELECT `livesModeWrong` FROM `users` WHERE `usertgid` = ?", (user_id,))
        result = self.cursor.fetchone()
        if result:
            return result[0]
